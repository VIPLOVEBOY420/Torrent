...
async def link_regex(c,m):
    try:
        ...
        for link in torrent_link:
            ...
            msg += f"**Name : {text}**\n**Link:** {tor}\n\n-\n\n"
        if msg == "":
            await m.send_message(-1002274069391, "No Torrents Found")
            await m.message.delete()
        elif msg != "":
            reply_text = f"{msg}"
            await c.send_photo(-1002274069391, p, caption=heading)
            await c.send_message(-1002274069391, reply_text)
            await txt.delete()

    except Exception as e:
        print(e)
        await c.send_message(-1002274069391, 'Some error occurred')
        await txt.delete()
